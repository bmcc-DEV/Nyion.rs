...
use swamp_gpu::{gpu_init, gpu_stream_create, gpu_upload_weights, gpu_swamp_init, gpu_swamp_launch, gpu_swamp_enqueue, gpu_swamp_shutdown, gpu_copy_to_device, gpu_copy_to_host, gpu_swamp_readback};
...
fn main() -> Result<()> {
    ...
    let mut kernel_stream = unsafe { std::mem::zeroed() };
    let mut data_stream = unsafe { std::mem::zeroed() };
    if let Ok(_) = gpu_init() {
        if let Ok(ks) = gpu_stream_create() { kernel_stream = ks; }
        if let Ok(ds) = gpu_stream_create() { data_stream = ds; }
        ...
        if gpu_upload_weights(all_w.as_ptr(), &mut d_w, total_w, data_stream).is_ok()
            && unsafe { gpu_swamp_init(&mut d_ring, &mut d_state, &mut d_shutdown, data_stream).is_ok() }
        {
            unsafe { let _ = gpu_swamp_launch(d_ring, d_w, d_state, d_shutdown, kernel_stream); }
            println!("  Swamp Continuum: GPU ativo (kernel stream + data stream)");
            _use_gpu = true;
        } else { eprintln!("  GPU init failed — using CPU"); }
    } else { eprintln!("  GPU not available — using CPU"); }
        ...
        for _step in 0..decode_steps {
            for l in 0..num_layers {
                ...
                if _use_gpu {
                    ...
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0, w_base + ring.q_off as i32, 0x10000, ring.q_nr as i32, (ring.q_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0, w_base + ring.k_off as i32, 0x20000, ring.k_nr as i32, (ring.k_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0, w_base + ring.v_off as i32, 0x30000, ring.v_nr as i32, (ring.v_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                ...
                if _use_gpu {
                    ...
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0x40000, w_base + ring.o_off as i32, 0x50000, ring.o_nr as i32, (ring.o_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                ...
                if _use_gpu {
                    ...
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0x60000, w_base + ring.gate_off as i32, 0x70000, ring.gate_nr as i32, (ring.gate_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0x60000, w_base + ring.up_off as i32, 0x80000, ring.up_nr as i32, (ring.up_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
                ...
                if _use_gpu {
                    ...
                    let _ = gpu_swamp_enqueue(d_ring, 0, l as i32, 0x90000, w_base + ring.down_off as i32, 0xA0000, ring.down_nr as i32, (ring.down_nc / 256) as i32, local_tail, data_stream); local_tail += 1;
